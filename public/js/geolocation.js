/**
 * Created by stevezhang on 2015-09-13.
 */
...
//construct province and city object: comefrom
function comefrom(theprovince, thecity) {
    this.theprovince = theprovince; this.thecity = thecity;
}

//instantiate 36 where object
...
// user select and initialize function
function geolocationselect() {
    with(document.suForm.province) {
        var selectedProvince = options[selectedIndex].value; }
    //selectedProvince is user selection of province
    for(i = 0;i < where.length;i ++) {
        if (where[i].theprovince == selectedProvince) {
            withCity = (where[i].thecity).split("|");
            //withCity is array of cities within given province
            for(j = 0;j < withCity.length;j++) {
                with(document.suForm.city) {
                    length = withCity.length;
                    options[j].text = withCity[j];
                    options[j].value = withCity[j];
                    var selectedCity=options[selectedIndex].value;}
                //selectedCity is the user selected cities
            }
            break;
        }
    }
    document.suForm.address.value=selectedProvince+selectedCity;
}
...
function geolocationInit() {
    with(document.suForm.province) {
        ...
        for(k=0;k<where.length;k++) {
            options[k].text = where[k].theprovince;
            options[k].value = where[k].theprovince; }
        options[selectedIndex].text = where[0].theprovince;
        options[selectedIndex].value = where[0].theprovince;
    }
    with(document.suForm.city) {
        withCity = (where[0].thecity).split("|");
        length = withCity.length;
        for(l=0;l<length;l++) {
            options[l].text = withCity[l];
            options[l].value = withCity[l]; }
        options[selectedIndex].text = withCity[0];
